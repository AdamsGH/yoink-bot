...
interface UserPanelProps {
  /** Compact trigger for sidebar footer / mobile tab */
  compact?: boolean
}

export function UserPanel({ compact = false }: UserPanelProps) {
  ...
  const trigger = compact ? (
    // Mobile tab bar: just avatar + "Me" label
    <Button variant="ghost" className="flex flex-1 flex-col items-center justify-center gap-1 py-2 h-auto text-xs text-muted-foreground hover:text-foreground">
      <Avatar photoUrl={photoUrl} name={name} size={24} />
      <span className="leading-none">{name.split(' ')[0]}</span>
    </Button>
  ) : (
    // Sidebar footer: avatar + name + role in a row
