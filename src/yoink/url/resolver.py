...
def resolve(
    ...
    engine = _pick_engine(domain, domain_cfg)
...
def _pick_engine(domain: str, cfg: DomainConfig) -> Engine:
    if domain_matches(domain, cfg.ytdlp_only):
        return Engine.YTDLP
    if domain_matches(domain, cfg.gallery_only):
        return Engine.GALLERY_DL
    # Check gallery path prefixes
    for pattern in cfg.gallery_paths:
        if f"{domain}/" in pattern or domain in pattern:
            return Engine.GALLERY_DL
