...
from yoink.commands.media import _is_group
from yoink.url.clip import ClipSpec, parse_time
...
async def _cmd_cut(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not url:
        if _is_group(update):
            await update.message.reply_html("Usage: <code>/cut &lt;url&gt; [start] [end]</code>")
        else:
            await update.message.reply_html(
                "✂️ <b>/cut</b>  - trim a video to a time range.\n\n"
                "<b>Usage:</b>\n"
                "<code>/cut URL</code>  - interactive mode\n"
                "<code>/cut URL 01:23 02:45</code>  - direct cut\n"
                "<code>/cut URL 01:23 90</code>  - start + duration in seconds"
            )
        return
    ...
    if start is not None and end is not None:
        if end <= start:
            ...
            return
        from yoink.commands.url_handler import _run_download
    ...
    if start is not None:
        prompt = await update.message.reply_html(
            f"✂️ Start: <code>{_fmt(start)}</code>\n\n"
            "Send <b>end time</b> or <b>duration</b>:\n"
            "<code>02:45</code>  or  <code>90</code> (seconds)"
        )
    ...
    from yoink.commands.ask_menu import show_menu
...
async def handle_cut_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if step == "start":
        prompt = await update.message.reply_html(
            f"✂️ Start: <code>{_fmt(value)}</code>\n\n"
            "Send <b>end time</b> or <b>duration</b>:\n"
            "<code>02:45</code>  or  <code>90</code> (seconds)"
        )
    ...
    if end_sec <= start_sec:
        await update.message.reply_html(
            f"❌ End (<code>{_fmt(end_sec)}</code>) must be after "
            f"start (<code>{_fmt(start_sec)}</code>). Try again:"
        )
        return True
